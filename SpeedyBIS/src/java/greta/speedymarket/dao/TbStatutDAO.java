...
public class TbStatutDAO {
    ...
    public List<TbStatut> findAll() {
     ...
     public List<TbStatut> findById(Integer id) {
        
         List<TbStatut> leStatut = new ArrayList<>();
        Session session = HibernateUtil.getSessionFactory().getCurrentSession();
        session.beginTransaction();
        leStatut = session.createQuery("select c from TbStatut WHERE idStatut = id").list();
        session.close();
